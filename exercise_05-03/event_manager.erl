...
terminate([]) -> []; terminate([{Handler, Data}|Rest]) ->
...
handle_msg({swap_handler, OldHandler, NewHandler}, LoopData) ->
  swap_handler({swap_handler, OldHandler, NewHandler}, LoopData);
...
swap_handler({swap_handler, OldHandler, NewHandler}, LoopData) -> 
  % {ok, [{Handler, Handler:init(InitData)}|LoopData]};
  {{data, InitData}, LoopDataAfterDelete} = handle_msg({delete_handler, OldHandler}, LoopData),
  {Replay, LoopDataAfterInsert} = handle_msg({add_handler, NewHandler, InitData}, LoopDataAfterDelete),
  {Replay, LoopDataAfterInsert}.
